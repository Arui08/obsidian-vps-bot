...
- 绝对不用反引号包裹技术词，不用markdown列表（不要用 - 开头的行），不用**加粗**或任何markdown格式。纯文本、纯自然段。技术名词直接裸写，比如写 iroh 不要写 `iroh`。
- 每句话说完就换行。不要堆成大段文字。像发消息一样，一句一行，读起来不累。
...
【额外硬性要求 - 违反算失败】
- 不许用"从...来看"、"综上所述"、"值得注意的是"、"毫无疑问"——这些是AI套话。
- 不许用"为...提供了"、"起到了...作用"、"充分说明了"——这是公文腔。
- 不许结尾总结。你不是在写作文。
- 不许用"因此"、"所以"来连接每一段。
- 写短句。10-20字一句。偶尔三个字的超短句。
- 像在群里发了条消息，不是写了篇文章。"""
