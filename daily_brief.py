...
ANGLE_POOL = {
    "ai": [
        "打工人视角：这东西怎么影响普通人的工作饭碗和效率",
        "老板视角：它会不会变成降本增效的新借口",
        "产品落地视角：别讲模型多强，只讲能不能真的干活",
        "开发者视角：它怎么改变写代码、调试、查资料这些日常动作",
        "普通人焦虑视角：AI不是科幻，它已经开始挤进每天的工作流",
    ],
    "crypto": [
        "老韭菜盘感：涨跌背后是情绪，不要无脑冲",
...
def _pick_angle(item: dict) -> str:
    """不用随机数，按日期+标题稳定挑一个叙事角度，保证每天有变化但可复现。"""
    pool = ANGLE_POOL.get(item.get("domain"), [])
    if not pool:
        return "人的观点/吐槽视角：不要像资讯摘要，要像一个人在表达判断"
    seed = f"{today_str()}|{item.get('domain','')}|{item.get('title','')}"
...
def make_brief_tweet(item: dict) -> str:
    ...
    if item.get("link_kind"):
        link_hint = f"\n链接性质：{item['link_kind']}（写的时候要把链接当成'可直接用/可下载/可 star 的入口'，行动召唤要具体，不要泛泛说链接在下面）"

    domain_instruction = {
        "ai": "不要写成AI新闻摘要。要写成AI正在改变工作流、职业饭碗、老板算盘、普通人焦虑的短观点。",
        "tools": "不要上来介绍项目。先说它解决了什么烦人的破事，再说为什么值得试。GitHub/工具链接必须像真实入口一样处理。",
    ...
    prompt = f"""你是一个混迹中文 X/Twitter 的内容博主，具体角色：{style['persona']}。风格像"鸟哥|蓝鸟会"那种——口语化、有钩子、有人味，但不要每条都一个套路。
...
铁律（违反任何一条都算失败）：
1. 全文严格不超过300个中文字符（包括链接和hashtag），宁可写短，不要写满。
2. 最多4段，每段1-2句，段落之间必须空一行。结构：人的钩子/痛点 → 说人话讲清楚发生了什么 → 1个具体细节/数据/槽点 → 自然行动召唤。
3. 绝对不用 emoji，一个都不许。要列点用普通 "1. 2."。
4. 开头第一句必须从人的感受、痛点、场景、吐槽或判断切入。参考方向：{style['vibes']}。
5. 严禁以下开头或近似表达："一觉醒来HN又炸了"、"HN上又吵起来了"、"HackerNews刷到"、"Reddit评论区炸了"、"今天看到一个"、"这个项目"、"近期发现"、"分享一个"、"今天给大家推荐"、"为大家介绍"。
6. 全文最多只能提一次信息源；能不提 HN/Reddit 就不提。即使要提，也只能放在中间作为数据来源，不许当开头。
7. 如果有具体数字（涨幅、价格、stars、点赞数、评论数），必须出现至少一个，数字是说服力。
8. 纯文本，不用 markdown 的 # 标题、不用 ** 加粗、不用 > 引用块、不用反引号。
9. 行动召唤要像真人说话，可以是"我先 mark 了"、"准备丢进去试一下"、"这事你们怎么看"、"别光听我吹，自己试"，不要太正式。
10. 行动召唤之后单独一行附链接：{item['url']}
11. 最后另起一行，3-4 个相关中文 hashtag（# 开头空格分隔），贴合主题。

主题领域：{style['label']}
信息源数据：{item.get('extra','')}{link_hint}
标题：{item['title']}
链接：{item['url']}

直接输出推文正文，不要"好的我来写"这种废话开头。输出前自查一遍：删掉背景解释、删掉废话、控制在300字以内。"""
