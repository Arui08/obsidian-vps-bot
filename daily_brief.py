...
SHAPE_POOL = [
    "三点拆解型：开头钩子后，用 1️⃣ 2️⃣ 3️⃣ 分别写发生了什么、为什么重要、我怎么看/怎么用",
    "打工人翻译器型：把表面新闻翻译成普通人的真实感受，再用 1️⃣ 2️⃣ 3️⃣ 拆开讲",
    "反直觉型：先说表面看是什么，再说实际上是什么，最后说最容易被忽略的点",
    "痛点故事型：先讲以前怎么麻烦/焦虑，再讲这件事怎么改变工作流或盘面判断",
]


def _stable_pick(pool: list, item: dict, salt: str) -> str:
    if not pool:
...
def _pick_shape(item: dict) -> str:
    """按日期+标题稳定挑内容形态，避免每条都一个模板。"""
    return _stable_pick(SHAPE_POOL, item, "shape")
...
2. 段落之间必须空一行。必须有 1️⃣ 2️⃣ 3️⃣ 三个核心点，每个核心点最多一句，句子要短。
3. 允许且只允许使用 1️⃣ 2️⃣ 3️⃣ 这三个序号 emoji；禁止其他 emoji（比如火箭、火、笑脸、箭头、手指等）。
...
6. 不要写 HN/Reddit 的点赞数、评论数、热度数字，也不要写"评论区炸了"。这些是信息源痕迹，容易暴露AI感。可以使用真正有内容价值的数字：价格、涨幅、版本号、节省比例、stars、token、时间成本等。
7. 不要直接复述标题。要提炼 3 个核心内容：发生了什么、为什么重要、普通人/开发者/交易者该怎么看。
8. 纯文本，不用 markdown 的 # 标题、不用 ** 加粗、不用 > 引用块、不用任何反引号；技术词直接裸写，比如 fork()+exec()。
9. 结尾必须有一个真人式互动/行动召唤：比如"这事你怎么看"、"我先 mark 周末试"、"你会接还是等回踩"、"别光听我吹，自己跑一遍"。
...
直接输出推文正文，不要"好的我来写"这种废话开头。输出前自查：去掉HN/Reddit痕迹、去掉废话、保留人的判断和3个核心点。"""
